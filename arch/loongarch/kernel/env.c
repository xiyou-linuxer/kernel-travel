...
void show_memory_map(void){
    ...
    for (int i = 0; i < loongson_mem_map->map_count; i++) {
        ...
        switch (mem_type) {
            ...
            pr_info("区域 %d: 起始地址: %llx, 大小: %llx, 类型: %u,主内存\n", i, mem_start,
                mem_size, mem_type);
                //memblock_set_node(mem_start, mem_size, &memblock.memory, 0);
                break;
            case ADDRESS_TYPE_ACPI:
                //memblock_add(mem_start, mem_size);
                pr_info("区域 %d: 起始地址: %llx, 大小: %llx, 类型: %u,加入\n", i, mem_start,
                    mem_size, mem_type);
                //memblock_set_node(mem_start, mem_size, &memblock.memory, 0);
                break;
            case ADDRESS_TYPE_RESERVED:
                //memblock_reserve(mem_start, mem_size);
                pr_info("区域 %d: 起始地址: %llx, 大小: %llx, 类型: %u,保留\n", i, mem_start,
                    mem_size, mem_type);
                break;
            default:
                pr_info("区域 %d: 起始地址: %llx, 大小: %llx, 类型: %u,0000\n", i, mem_start,
                mem_size, mem_type);
                continue;
        }
    }
